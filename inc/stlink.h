...
#include "../src/uartlib-serial/modbus_wrapper.h"
...
typedef struct uart_settings
{
    ...
    uint32_t work_baudRate;

    uint8_t init_stopBits;
    uint8_t init_parity;
    uint8_t work_stopBits;
    uint8_t work_parity;
...
struct _stlink {
    ...
    uart_settings_t dbgSettings;
    uint8_t isConnected;
    //void *modbus;
    int32_t modbus;
    uint8_t msgStatus; // [-1 for error, 0 for okay]
    struct stlink_libsg * backend_data;
    // NO STLINK
    // Modbus modbus; // modbus wrapper
    uint8_t cmd; // command enum (from uart.h)
    uint32_t uid[3]; // Device uart id || INIT with zeros 
    //
